(()=>{
  ...
  app.toggleLayer=button=>{
    ...
    if(key==='parking'&&app.enabled.has('parking')){
      app.setStatus(`Паркинги: зареждам всички OSM паркинги в ${app.labelRadius(app.effectiveRadius())}…`,'info',true);
      app.layers.parking.clearLayers();s.parkingCandidates=[];app.scheduleLoad(30);return;
    }
  ...
  app.fetchLayer=async(key,queryRadius,requested,version)=>{
    ...
    try{
      ...
      const data=await response.json();if(!response.ok)throw new Error(data.error||'Overpass error');
  ...
  app.loadSmartData=async(force=false)=>{
    ...
    const wideParking=app.enabled.has('parking')&&requested>app.QUERY_CAP;
    const localKeys=[...app.enabled].filter(key=>!(key==='parking'&&wideParking));
    const queryRadius=Math.max(25,Math.min(requested,app.QUERY_CAP)),centerLabel=s.activeDestination?'дестинацията':'теб',results=[];
    app.setStatus(wideParking?`Зареждам всички паркинги в ${app.labelRadius(requested)} около ${centerLabel}…`:`Обновявам ${localKeys.length} слоя около ${centerLabel}…`,'info',true);

    for(let index=0;index<localKeys.length;index+=2){
      const batch=localKeys.slice(index,index+2);results.push(...await Promise.all(batch.map(key=>app.fetchLayer(key,queryRadius,requested,version))));
    }
    if(wideParking)results.push(await app.loadWideParking(requested,version));
    if(version!==s.requestVersion){s.loading=false;return}

    const failed=results.filter(item=>item?.error).length;
    const failedRegions=results.reduce((sum,item)=>sum+(item?.failedCells||0),0);
    const loaded=results.filter(item=>Number.isFinite(item?.count)),total=loaded.reduce((sum,item)=>sum+item.count,0);
    s.lastLoadAt=Date.now();
    const scope=selected>=250000?'цела България':`радиус ${app.labelRadius(requested)}`;
    app.$('counts').innerHTML=[...app.enabled].map(key=>{const result=results.find(item=>item?.key===key);return`<div class="count">${app.defs[key].glyph} ${app.defs[key].label}: <b>${Number.isFinite(result?.count)?result.count:'—'}</b></div>`}).join('');
    app.$('meta').textContent=s.activeDestination?`${scope} около дестинацията · ${total} OSM обекта · свободните места не са live`:`${scope} · ${total} OSM обекта · не са live`;
    app.$('last-update').textContent=`Обновено ${new Date().toLocaleTimeString('bg-BG',{hour:'2-digit',minute:'2-digit'})}`;
    if(failed||failedRegions)app.setStatus(`Заредени ${total} обекта. ${failedRegions||failed} региона не отговориха — натисни P за повторно зареждане.`,'error',true);
    else app.setStatus(`Готово: ${total} OSM обекта в ${scope}.`,'success');

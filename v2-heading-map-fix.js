(()=>{
  ...
  const VERSION='5.0.0';
  const CALIBRATION_KEY='sf_v2_heading_calibration_v5';
  const LEGACY_KEYS=['sf_v2_heading_calibration_v2','sf_v2_heading_calibration_v3','sf_v2_heading_calibration_v4'];
  ...
  const controller={
    version:VERSION,compassRaw:null,compassHeading:null,compassAccuracy:null,compassConfidence:0,compassAt:0,
    gpsReported:null,movementCourse:null,movementConfidence:0,movementAt:0,absoluteHeading:null,
    visualBearing:0,targetBearing:0,source:'waiting',confidence:0,calibrationOffset:0,calibrationSamples:[],
    lastFix:null,lastFrameAt:0,initialized:false,orientationListening:false,orientationPermission:'unknown',
    rotationSupported:false,warnedMissingRotation:false,debugNode:null
  };
  ...
  const onOrientation=event=>{
    ...
    const absolute=event.type==='deviceorientationabsolute'||event.absolute===true||finite(event.webkitCompassHeading);
    const confidence=accuracy!==null?clamp(1-accuracy/90,.12,1):(absolute?.72:.34);
  ...
  const startOrientationListening=()=>{
    if(controller.orientationListening||!('DeviceOrientationEvent'in window))return false;
    window.addEventListener('deviceorientationabsolute',onOrientation,true);
    window.addEventListener('deviceorientation',onOrientation,true);
    controller.orientationListening=true;
    controller.orientationPermission='granted';
    return true;
  };
  ...
  const requestHeadingPermission=async()=>{
    ...
    if(typeof DeviceOrientationEvent.requestPermission==='function'){
      try{
        ...
        controller.orientationPermission=result;
        return result==='granted'?startOrientationListening():false;
      }catch{
        controller.orientationPermission='unknown';
        return false;
      }
    }
  ...
  const selectHeading=(state,user)=>{
    ...
    if(speed>=7&&movementFresh){
      return{heading:gps===null?controller.movementCourse:lerpAngle(controller.movementCourse,gps,.28),source:'movement',confidence:clamp(.58+controller.movementConfidence*.38,0,1)};
    }
    if(speed>=5&&gps!==null){
      return{heading:compassFresh?lerpAngle(controller.compassHeading,gps,.68):gps,source:'gps',confidence:compassFresh?clamp(.58+controller.compassConfidence*.2,0,1):.64};
    }
  ...
  const keepArrowForward=()=>{
    const marker=window.SFV2?.state?.userMarker?.getElement?.()?.querySelector('.user-position-marker');
    if(marker){
      marker.classList.add('sf-heading-up');
      marker.style.setProperty('--heading','0deg');
      marker.style.setProperty('--sf-correct-heading','0deg');
      marker.style.setProperty('--sf-final-heading','0deg');
    ...
    if(hud){
      hud.style.transform='rotate(-90deg)';
      hud.dataset.headingSource=controller.source;
    }
  };

  const applyMapBearing=(state,elapsed)=>{
    const map=state?.map;
    controller.rotationSupported=Boolean(window.SFMapRotationPluginReady&&map&&typeof map.setBearing==='function'&&typeof map.getBearing==='function');
    keepArrowForward();
    if(!controller.rotationSupported){
      if(!controller.warnedMissingRotation){controller.warnedMissingRotation=true;console.error('[SF heading] map rotation plugin unavailable')}
      return;
    }
    if(state.followUser===false)return;
    controller.targetBearing=normalize(controller.absoluteHeading);
    if(!controller.initialized){
      controller.visualBearing=finite(map.getBearing())?normalize(map.getBearing()):controller.targetBearing;
      controller.initialized=true;
    }
    const difference=shortestDelta(controller.visualBearing,controller.targetBearing);
    const speed=Math.max(0,Number(state?.arrowEngine?.smoothedSpeed??state?.user?.speed??0));
    const maxRate=speed>=7?300:speed>=2?210:105;
    const response=speed>=7?9:speed>=2?6.8:3.8;
    const step=clamp(difference*clamp(response*elapsed,0,1),-maxRate*elapsed,maxRate*elapsed);
    controller.visualBearing=normalize(controller.visualBearing+step);
    if(Math.abs(difference)<.08)controller.visualBearing=controller.targetBearing;
    map.setBearing(controller.visualBearing);
    state.mapBearing=controller.visualBearing;
    state.navigationMapBearing=controller.visualBearing;
    document.body.classList.add('heading-up-map');
  };
  ...
  const renderDebug=()=>{
    ...
    if(!node)return;
    node.textContent=[
      `Heading-up Engine ${VERSION}`,
      `rotation ${controller.rotationSupported?'ready':'missing'}`,
      `source ${controller.source} · confidence ${Math.round(controller.confidence*100)}%`,
      `absolute ${finite(controller.absoluteHeading)?controller.absoluteHeading.toFixed(1):'—'}°`,
      `map ${controller.visualBearing.toFixed(1)}° · target ${controller.targetBearing.toFixed(1)}°`,
      `compass ${finite(controller.compassHeading)?controller.compassHeading.toFixed(1):'—'}°`,
      `movement ${finite(controller.movementCourse)?controller.movementCourse.toFixed(1):'—'}°`,
      `gps ${finite(controller.gpsReported)?controller.gpsReported.toFixed(1):'—'}°`,
  ...
  const frame=timestamp=>{
    ...
    if(user&&finite(user.lat)&&finite(user.lon)){
      ...
      if(selected&&finite(selected.heading)){
        ...
        applyMapBearing(state,elapsed);
      }else keepArrowForward();
    }
    renderDebug();
  ...
  const selfTest=()=>{
    const checks={
      north:normalize(0)===0,east:normalize(90)===90,wrapPositive:shortestDelta(350,10)===20,
      wrapNegative:shortestDelta(10,350)===-20,nullIsNotHeading:!finite(null),pluginDetected:Boolean(window.SFMapRotationPluginReady)
    };
  ...
  const expose=()=>{
    ...
    app.headingDiagnostics=()=>({
      version:VERSION,source:controller.source,confidence:Math.round(controller.confidence*100),absolute:controller.absoluteHeading,
      mapBearing:controller.visualBearing,targetBearing:controller.targetBearing,compass:controller.compassHeading,
      movement:controller.movementCourse,gps:controller.gpsReported,calibrationOffset:controller.calibrationOffset,
      screenAngle:screenAngle(),rotationSupported:controller.rotationSupported,
      orientationListening:controller.orientationListening,orientationPermission:controller.orientationPermission
    });
  ...
  screen.orientation?.addEventListener?.('change',()=>{controller.compassHeading=null;controller.compassAt=0;controller.initialized=false});
  document.addEventListener('visibilitychange',()=>{controller.lastFrameAt=0;if(!document.hidden)controller.initialized=false});

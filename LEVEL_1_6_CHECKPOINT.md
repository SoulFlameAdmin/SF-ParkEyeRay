...
- Onboarding now links `manifest.webmanifest` and loads `/pwa-register.js`.
- `/pwa-register.js` safely registers `/sw.js` with application-wide scope and does not break unsupported browsers.
- Preview acceptance now verifies the manifest link, registration script endpoint, registration call and scope.
...
- Previous offline-resilience batch passed GitHub Actions before this batch.
- The new PWA activation commits require their own GitHub Actions and exact Vercel Preview verification before acceptance.
...
- `09b3012e0eb50edf61addbf28818b94b2787fb54` — cache `/app` and return honest offline API failures.
- `975016be5cd981e5ce750df0ce3c8b99f2693c31` — verify offline PWA behavior.
...
The runner verifies `/`, `/pwa-register.js`, `/app`, `/manifest.webmanifest`, `/sw.js`, `/offline.html`, invalid `/api/geocode` input and invalid `/api/route` input. Static checks prove that registration is wired into the delivered HTML, but they do not prove a physical browser installation, service-worker lifecycle or offline navigation session.

## Next checks before production
1. Wait for and verify GitHub Actions on the exact latest checkpoint commit.

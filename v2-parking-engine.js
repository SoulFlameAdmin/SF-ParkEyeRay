(()=>{
  ...
  app.engineParking=record=>{
    ...
    const straight=Number.isFinite(Number(record.distance))?Number(record.distance):app.distance(point,s.destination);
  ...
  app.fetchParkingEngine=async(radius,signal)=>{
    const params=new URLSearchParams({lat:String(s.destination.lat),lon:String(s.destination.lon),radius:String(radius),limit:'120'});
  ...
  app.findParkings=async()=>{
    ...
    try{
      ...
      for(const radius of app.RADII){
        ...
        payload=await app.fetchParkingEngine(radius,controller.signal);usedRadius=radius;
        if(payload.parkings.length>=5||radius===app.RADII.at(-1))break;
      }
      if(controller.signal.aborted)return;
      s.parkings=payload.parkings.map(app.engineParking).filter(Boolean);app.sortParkings();app.renderParkingMarkers();app.renderParkings();
  ...
  app.renderParkingMarkers=()=>{
    ...
    s.parkings.forEach(item=>{
      ...
      marker.bindPopup(`<b>${app.safe(item.name)}</b><br>${app.safe(item.kind)}<br>${app.formatDistance(item.walk)} до дестинацията<br><small>${app.safe(app.parkingSourceLabel(item))}</small>`);
  ...
  app.renderParkings=()=>{
    ...
    if(!s.parkings.length){
      app.renderParkingMessage('🅿️','Няма налични резултати','SmartCity базата може още да няма импортирани всички физически и улични паркинги.',{actionLabel:s.destination?'Обнови':'Търси място',onAction:s.destination?app.findParkings:()=>app.$('search-input').focus()});
      return;
    }
